document.addEventListener("DOMContentLoaded", () => {
	...
	const canMl = Number(document.querySelector("#canMl").innerHTML); // 사용가능한 적립금금액

	const discountRadioGroup = document.querySelectorAll("input[name='discount-radio-group']"); // 적립금 선할인 라디오그룹
    const preDiscountPrice = Number(document.querySelector("#pre-discount-price").innerHTML); // 선할인금액
	...
	/* 적립금 선할인을 수행하는 함수 */
	function discountPreMileage(discountRadioGroup) {
		discountRadioGroup.forEach((discountRadio) => {
			discountRadio.addEventListener("change", (e) => {
				let current = e.currentTarget;

				// 총금액을 가져온다.
				let currentTotalPrice =  Number(totalPriceTag.innerHTML);

				// 총적립금할인 합계 금액을 가져온다.
				let totalMileageDiscount = Number(totalMileageDiscountTag.innerHTML);
				...
				if(current.checked) {
					...
					// 할인금액을 업데이트한다.
					totalMileageDiscountTag.innerHTML = totalMileageDiscount.toString();
					totalPriceTag.innerHTML = currentTotalPrice.toString();
				}
			});
		});
	}
	...
	/* 보유적립금을 사용하는 함수 */
	function useMileage() {
		memMlTag.addEventListener("blur", () => {	// 보유적립금 태그의 포커스를 잃을때
			// 입력값을 가져온다.
			let inputMl = Number(memMlTag.value);
			...
			// 적립금할인 정보를 업데이트한다.
			let prevToatlMileageDiscountPrice = Number(totalMileageDiscountTag.innerHTML);
			totalMileageDiscountTag.innerHTML = (inputMl - prevMl + prevToatlMileageDiscountPrice).toString();
...
//			let selectedValue = this.options[this.selectedIndex].value;
			...
			// 쿠폰할인금액을 업데이트 한다.
			totalDiscountCouponPrice.innerHTML = newCounponDiscountPrice.toString();
	...
	/* 적립금 선할인 함수실행 */
	discountPreMileage(discountRadioGroup);
...
//		    amount: 100, // 테스트 가격
			...
			console.log(resp);
		   	...
		   	if(resp.success) {
				...
				let addSavings = Number(document.querySelector("#pre-discount-price").innerHTML); // 구매적립금
				...
				let memMlMoney = Number(memMlTag.value); // 사용한 보유적립금

				let couponId = myCouponList.options[myCouponList.selectedIndex].value; // 쿠폰아이디
				let couponDiscountPrice = Number(totalDiscountCouponPrice.innerHTML); // 쿠폰할인금액

				let totalMileageDiscountPrice = Number(totalMileageDiscountTag.innerHTML); // 총적립금할인금액
