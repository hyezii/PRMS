	                                ...
	                                <div class="col-md-12 col-sm-12 mb-6 ms-blog-block">
	                                    <div class="ms-blog-item">
	                                        <div class="news-info" style="padding: 0 24px 10px 24px;">
	                                            <div class="detail">
	                                            <%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
	                                                <label id="tmstyle">${res.resBeautyRegistDe}</label>
<%-- 	                                                <label id="tmstyle"><fmt:formatDate value="${res.resBeautyRegistDe}" pattern="yyyy-MM-dd HH:mm:ss" /></label> --%>
	                                                <h3 class="resinfotitle" style="margin-top: 0;">${res.franchiseNm }</h3>
	                                                <div class="row" style="margin-left:0">
	                                                <p class="text-length col" style="font-size:15px;">예약시간 : ${res.resBeautyDe } ${res.resBeautyTime }
	                                                	<br>예약상태 : ${res.cmmnCodeNm }
	                                                <c:set var="difference" value="${res.resBeautyPrice - res.resOptionPrice}" />
	                                                	<br>미용옵션 : ${res.resOption }
	                                                	<br>미용가격 : <fmt:formatNumber value="${res.resOptionPrice }" />원
	                                                		<c:if test="${difference != 0}">
	                                                			(+ 몸무게 추가 <fmt:formatNumber value="${difference }" />원)
	                                                		</c:if>
	                                                <c:if test="${res.cmmnCodeNm eq '예약완료'}">
	                                                <div class="more-info col" id="cancel">
	                                                    <input type="button" value="예약취소" id="allbtn" class="cancelBtn" data-index="${res.resBeautyNo}"/>
	                                                </div>
	                                                <form action="${cPath }/market/member/${res.resBeautyNo}/cancel" method="post" class="cancelForm" data-index="${res.resBeautyNo}">
														<input type="hidden" name="resBeautyNo" value="${res.resBeautyNo}" required />
													</form>
	                                                </c:if>
<%-- 	                                                <c:if test="${res.cmmnCodeNm eq '이용완료'}"> --%>
<!-- 													<div class="more-info col" id="cancel"> -->
<%-- 	                                                    <input type="button" value="리뷰작성" id="allbtn" class="reviewBtn" data-index="${res.resBeautyNo}"/> --%>
<!-- 	                                                </div> -->
<%-- 	                                                <form action="${cPath }/" method="post" class="reviewForm" data-index="${res.resBeautyNo}"> --%>
<%-- 														<input type="hidden" name="resBeautyNo" value="${res.resBeautyNo}" required /> --%>
<%-- 													</form> --%>
<%-- 	                                                </c:if> --%>
	                                                </div>
...
document.querySelectorAll(".cancelBtn").forEach((button) => {
    button.addEventListener("click", () => {
        ...
        if (!cancelForm) {
            console.error("Cancel form not found for index:", index);
            return;
        }
        Swal.fire({
            title: '예약을 취소하시겠습니까?',
