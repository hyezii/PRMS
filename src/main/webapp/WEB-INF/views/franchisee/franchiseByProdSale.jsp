...
<c:set value="${franTop5List }" var="frantop" />
...
<div class="card p-3 mb-5" style="display: flex;flex-direction: row;">
		...
		<div class="card-body" >
			...
			<div class="d-flex align-items-center" style="margin-bottom: 20px;">
				...
				<div class="flex-1 px-3">
					<h6 class="mb-0 fw-semibold d-flex align-items-center">${frantop[0].prodName }</h6>
				</div>
				<fmt:formatNumber value="${frantop[0].prodTotal  }" pattern="#,###" var="prodTotal0" />
				<div><span>${prodTotal0}원</span></div>
			</div>
			<div class="d-flex align-items-center" style="margin-bottom: 20px;">
				<i class="fas fa-circle" style="color: rgba(54, 162, 235, 0.2);font-size: x-large;"></i>
				<div class="flex-1 px-3">
					<h6 class="mb-0 fw-semibold d-flex align-items-center">${frantop[1].prodName }</h6>
				</div>
				<fmt:formatNumber value="${frantop[1].prodTotal  }" pattern="#,###" var="prodTotal1" />
				<div><span>${prodTotal1 }원</span></div>
			</div>
			<div class="d-flex align-items-center" style="margin-bottom: 20px;">
				<i class="fas fa-circle" style="color: rgba(75, 192, 192, 0.2);font-size: x-large;"></i>
				<div class="flex-1 px-3">
					<h6 class="mb-0 fw-semibold d-flex align-items-center">${frantop[2].prodName }</h6>
				</div>
				<fmt:formatNumber value="${frantop[2].prodTotal  }" pattern="#,###" var="prodTotal2" />
				<div><span>${prodTotal2 }원</span></div>
			</div>
			<div class="d-flex align-items-center" style="margin-bottom: 20px;">
				<i class="fas fa-circle" style="color: rgba(153, 102, 255, 0.2)	;font-size: x-large;"></i>
				<div class="flex-1 px-3">
					<h6 class="mb-0 fw-semibold d-flex align-items-center">${frantop[3].prodName }</h6>
				</div>
				<fmt:formatNumber value="${frantop[3].prodTotal  }" pattern="#,###" var="prodTotal3" />
				<div><span>${prodTotal3 }원</span></div>
			</div>
			<div class="d-flex align-items-center" style="margin-bottom: 20px;">
				<i class="fas fa-circle" style="color: rgba(255, 204, 182, 0.2);font-size: x-large;"></i>
				<div class="flex-1 px-3">
					<h6 class="mb-0 fw-semibold d-flex align-items-center">${frantop[4].prodName }</h6>
				</div>
				<fmt:formatNumber value="${frantop[4].prodTotal  }" pattern="#,###" var="prodTotal4" />
    ...
    <div class="col-xl-6 col-lg-12">
            <div class="card-body" style="padding-left: 30px;">
            	<h4 style="font-weight: 500;">상품별 매출 </h4>
...
<!--                         <th>#</th> -->
                ...
                <c:forEach items="${franByProdList }" var="franProd">
                    <tr>
<%--                         <td><fmt:formatNumber value="${sell[i].selngAmount }"/>원</td> --%>
                        <td>${franProd.prodName }</td>
                        <td>${franProd.cmmnCodeNm }</td>
                        <td>${franProd.prodCnt }개</td>
                        <fmt:formatNumber value="${franProd.prodTotal }" pattern="#,###" var="prodTotal" />
                        <td>${prodTotal }원</td>
                        <fmt:formatNumber value="${franProd.storePc }" pattern="#,###" var="storeTotalPc" />
                        <td>${storeTotalPc }원</td>
                        <fmt:formatNumber value="${franProd.prodTotal - franProd.storeTotalPc }" pattern="#,###" var="income" />
                        <td>${income }원</td>
                    </tr>
...
$(document).ready(function() {
	var franTop5ListArr = new Array();
	<c:forEach items="${franTop5List}" var="franTop5">
		franTop5ListArr.push({
			prodName : "${franTop5.prodName}"
			,prodTotal : "${franTop5.prodTotal}"
		});
	</c:forEach>
	...
	var doughnutChart = new Chart(ctx1, {
		...
		data : {
 			//labels : [ '상품', '미용예약', '호텔예약' ],
			datasets : [ {
				data : [ franTop5ListArr[0].prodTotal, franTop5ListArr[1].prodTotal, franTop5ListArr[2].prodTotal, franTop5ListArr[3].prodTotal , franTop5ListArr[4].prodTotal ],
...
<script src="${cPath }/resources/js/app/franchise/franchiseByProdSale.js" ></script>