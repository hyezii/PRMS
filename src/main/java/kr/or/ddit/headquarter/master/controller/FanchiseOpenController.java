...
public class FanchiseOpenController {
	@Autowired
	...
	public String FanchiseOpen(FoundDefaultVO foundVO) {
		...
		if (dnt > 0) {
			...
			franvo.setFranchiseNm("멍냥몰 "+city+"점");
			franvo.setFranchiseTel(foundVO.getFoundTelno());
			franvo.setFranchiseZip(foundVO.getFoundZip());
			franvo.setFranchiseAdd1(city+" "+qoduf[1]);
			franvo.setFranchiseAdd2(qoduf[2]+" "+foundVO.getFoundAdres2());
